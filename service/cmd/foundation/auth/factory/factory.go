...
import (
	...
	"github.com/fflow-tech/fflow/service/internal/foundation/auth/domain/service/command"
	"github.com/fflow-tech/fflow/service/internal/foundation/auth/domain/service/query"
...
func provideDomainService(container *dig.Container) {
	...
	container.Provide(sql.NewUserDAO)
	container.Provide(repo.NewUserRepo)
	container.Provide(sql.NewNamespaceDAO)
	container.Provide(repo.NewNamespaceRepo)
	container.Provide(sql.NewNamespaceTokenDAO)
	container.Provide(repo.NewNamespaceTokenRepo)

	container.Provide(command.NewRbacClient)
	container.Provide(command.NewRbacCommandService)
	container.Provide(command.NewAuthCommandService)
