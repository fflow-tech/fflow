...
// 这个方法提供了将 golang 的方法绑定到 goja 的 runtime 的方法，因为 codecc 的规则对函数进行了拆分
func Bind(rt *goja.Runtime, v interface{}, ctxPtr *context.Context) map[string]interface{} {
	...
	for i := 0; i < typ.NumMethod(); i++ {
		...
		fnT := fn.Type()
		numIn, hasError, wantsContext, wantsContextPtr := initVal(fnT)
		if numIn > 0 {
			in0 := fnT.In(0)
			switch in0 {
			case ctxT:
		...
		if hasError || wantsContext || wantsContextPtr {
			...
			fn = reflect.ValueOf(func(call goja.FunctionCall) goja.Value {
				...
				copyOverArgs(reservedArgs, numIn, fnT, args, call, isVariadic, rt)
	...
	return pointerToFields(val, exports)
}

func copyOverArgs(reservedArgs, numIn int, fnT reflect.Type, args []reflect.Value, call goja.FunctionCall,
	isVariadic bool, rt *goja.Runtime) {
	// Copy over arguments.
	for i := 0; i < numIn; i++ {
		if i < reservedArgs {
			continue
		...
		if isVariadic && i == numIn-1 {
			...
			args[i] = setArgs(reservedArgs, varArgsLen, i, call, T, rt)
			break
		}
...
func setArgs(reservedArgs, varArgsLen, i int, call goja.FunctionCall, t reflect.Type,
	rt *goja.Runtime) reflect.Value {
...
func initVal(fnT reflect.Type) (int, bool, bool, bool) {
	numIn := fnT.NumIn()
	numOut := fnT.NumOut()
	hasError := (numOut > 1 && fnT.Out(1) == errorT)
	wantsContext := false
	wantsContextPtr := false
	return numIn, hasError, wantsContext, wantsContextPtr
}

func pointerToFields(val reflect.Value, exports map[string]interface{}) map[string]interface{} {
	typ := val.Type()
